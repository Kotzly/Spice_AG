...
tam_pop=3
uid=0
data_filename = 'circuits/comparator.asc'
...
mos_types=['M0','M1','M2','M3','M4']
parameters=['W','L']
n_of_params=len(parameters)
...
def substitute_value(arq,mosfets,params,values,out):
#    flag=True
    with open(arq,'r') as data:
        text=data.readlines()
        words=''
        for line in text:
            if (not [-1 for m in mosfets]==[line.find(mos_type) for mos_type in mosfets]) and line.find('model')>0:                      
                seed=line
                mos=[i for i in range(0,len(mosfets)) if line.find(mosfets[i])>0][0]
                for param,value in zip(params,values[mos*n_of_params:(mos+1)*n_of_params]):
                    wordstemp=''
...
#                    ind=temp.index(' ')
                    ...
                    spc=temp.index(' ')                    
                    ind=[i for i in range(0,spc+1) if temp[i:spc].isalpha()]
                    if len(ind)==0:
                        ind=spc
                    else:
                        ind=ind[0]
...
class individual():
    def __init__(self):
        global uid,mos_types,parameters,n_of_params
        self.name=str(uid)
        self.mutex=threading.Semaphore(value=0)
        uid+=1
        self.cromo=[decimal.Decimal(random.random()) for i in range(0,len(parameters)*len(mos_types))]
        self.mos_types=mos_types
    ...
    def fitness(self):
        substitute_value(data_filename,
                         self.mos_types,
                         self.parameters,
                         [decimal.Decimal(round(dec,5)).to_eng_string()[:7] for dec in self.cromo],
#                         [str(round(dec,5)) for dec in self.cromo],
                          self.name+'.asc')
...
#        return np.log(1/abs(wanted_value-temp))
        ...
        temp = sum(data.values[0][-11:-1])/10
        return np.log(1/abs(vdd/2-temp))
